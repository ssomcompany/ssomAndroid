...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    private boolean isDriveStyle = false;
    protected boolean isDrawerStyle = false;
    ...
    public PullToRefreshBase(Context context) {
        super(context);
        mContext = context;
        init(context, null); //NOPMD
    }

    public PullToRefreshBase(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        init(context, attrs); //NOPMD
    }
    ...
    public final boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE: {
                if (mIsBeingDragged || isEmtpyVisible()) {
...
//                            }
    ...
    protected final LoadingLayout createLoadingLayout(Context context, Mode mode, TypedArray attrs) {
    ...
    protected abstract void setScrollEvent(T mRefreshableView);

    protected abstract void removeScrollEvent(T mRefreshableView);

    protected abstract void addFooter(T mRefreshableView);

    protected abstract void removeFooter(T mRefreshableView);

    protected abstract void setFooterVisible(boolean visible);
    ...
    protected void handleStyledAttributes(TypedArray a) {
        Util.callDummyRequestToBypassStupidPMD();
    }
    ...
    protected final void onSizeChanged(int w, int h, int oldw, int oldh) {
        if (DEBUG) {
            Log.d(TAG, String.format(Locale.getDefault(), "onSizeChanged. W: %d, H: %d", w, h));
        }
    ...
    protected final void setHeaderScroll(int value) {
        if (DEBUG) {
            Log.d(TAG, "setHeaderScroll: " + value);
        }
        ...
        if (USE_HW_LAYERS) {
            /**
             * Use a Hardware Layer on the Refreshable View if we've scrolled at all. We don't use them on the Header/Footer Views as they change
    ...
    protected void updateUIForMode() {
        if (mMode.equals(Mode.BOTH) || mMode.equals(Mode.PULL_FROM_END)) {
            addFooter(mRefreshableView);
            setScrollEvent(mRefreshableView);
        } else {
            removeFooter(mRefreshableView);
        }

        if (mMode.equals(Mode.PULL_FROM_START)) {
            setScrollEvent(mRefreshableView);
        }

        if (mMode.equals(Mode.DISABLED) && mSwipeRefreshLayout != null) {
            removeScrollEvent(mRefreshableView);
    ...
    private void addRefreshableView(Context context, T refreshableView) {
        ...
        parent.addView(refreshableView, ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT);
        mSwipeRefreshLayout = new SwipeRefreshLayout(context);
        mSwipeRefreshLayout.addView(parent, ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT);
        ...
        mRefreshableViewWrapper.addView(mSwipeRefreshLayout, ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT);
        ...
        switch (getPullToRefreshScrollDirection()) {
            case HORIZONTAL:
                addViewInternal(mRefreshableViewWrapper, new LayoutParams(0, LayoutParams.MATCH_PARENT,
                        1.0f));
                break;
            case VERTICAL:
            default:
                addViewInternal(mRefreshableViewWrapper, new LayoutParams(LayoutParams.MATCH_PARENT, 0,
                        1.0f));
                break;
        }
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrStyle)) {
            int ptrStyle = a.getInt(R.styleable.PullToRefresh_ptrStyle, 0);
            if (isDrive(ptrStyle)) {
                // isDriveStyle = true;
                Log.d(TAG, "ptrDrive style");
            } else if (isDrawer(ptrStyle)) {
                isDrawerStyle = true;
            } else {
                isDriveStyle = false;
                isDrawerStyle = false;
        ...
        if (isDriveStyle) {
            mHeaderLayout.setVisibleTopLine(false);
            mFooterLayout.setVisibleBottomLine(false);
            setHeaderLoadingText(context.getString(R.string.release_to_refresh));
            setFooterLoadingText(context.getString(R.string.settings_loading));
        } else if (isDrawerStyle) {
            mFooterLayout.setVisibleTopLine(false);
            mFooterLayout.setVisibleBottomLine(false);
        } else {
            mHeaderLayout.setVisibleBottomLine(false);
            mFooterLayout.setVisibleBottomLine(false);
        }
        /**
         * Styleables from XML
    ...
    private boolean isDrive(int style) {
        return style == 0x1;
    }

    private boolean isDrawer(int style) {
        return style == 0x2;
    }
    ...
    public void setEmptyView(View emptyView) {
        ...
        scrollview.addView(mEmptyView, 0,
                new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.MATCH_PARENT));
        ViewGroup parent = (ViewGroup) mRefreshableView.getParent();
    ...
    protected boolean isEmtpyVisible() {
    ...
    public final void setHeaderLoadingText(String text) {
        if (mHeaderLayout != null) {
            mHeaderLayout.setLoadingText(text);
        }
    }

    public void setHeaderLoadingText(@StringRes int rId) {
        if (mHeaderLayout != null) {
            ...
            mHeaderLayout.setLoadingText(mContext.getResources().getString(rId));
        }
    }

    public final void setFooterLoadingText(String text) {
        if (mFooterLayout != null) {
            mFooterLayout.setLoadingText(text);
        }
    }

    public void setFooterLoadingText(@StringRes int rId) {
        if (mFooterLayout != null) {
            if (mContext == null || mContext.getResources() == null) {
                Log.e(TAG, "getResources() is null!!!!");
                return;
            ...
            mFooterLayout.setLoadingText(mContext.getResources().getString(rId));
        }
    }

    public void setProgressDrawable(Drawable d) {
        if (mHeaderLayout != null) {
            mHeaderLayout.setIndeterminateDrawable(d);
        }
        if (mFooterLayout != null) {
            mFooterLayout.setIndeterminateDrawable(d);
        }
    }
    ...
    public static enum AnimationStyle {
        ...
        /**
         * Maps an int to a specific mode. This is needed when saving state, or inflating the view from XML where the mode is given through a attr
         * int.
         *
    ...
    private static boolean isFlip(int mode) {
        return 0x01 == mode;
    }
    ...
    public enum Mode {
        ...
        /**
         * Only allow the user to Pull from the start of the Refreshable View to refresh. The start is either the Top or Left, depending on the
         * scrolling direction.
         */
        PULL_FROM_START(0x1),

        /**
         * Only allow the user to Pull from the end of the Refreshable View to refresh. The start is either the Bottom or Right, depending on the
         * scrolling direction.
         */
        PULL_FROM_END(0x2),
    ...
    /**
     * Listener that allows you to be notified when the user has started or finished a touch event. Useful when you want to append extra UI events
    ...
    public static interface OnPullEventListener<V extends View> {
         ...
         * @param direction - One of {@link Mode#PULL_FROM_START} or {@link Mode#PULL_FROM_END} depending on which direction the user is pulling. Only
    ...
    /**
     * An advanced version of the Listener to listen for callbacks to Refresh. This listener is different as it allows you to differentiate between
