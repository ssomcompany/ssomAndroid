...
import com.ssomcompany.ssomclient.widget.pulltorefresh.PullToRefreshBase.State;

@TargetApi(9)
public final class OverscrollHelper {

    static final String TAG = OverscrollHelper.class.getSimpleName();
     ...
     * <p/>
     * This should only be used on AdapterView's such as ListView as it just calls through to overScrollBy() with the scrollRange = 0. AdapterView's
     ...
     * @param isTouchEvent - true if this scroll operation is the result of a touch event, passed through from from overScrollBy call
     */
    ...
    /**
     * Helper method for Overscrolling that encapsulates all of the necessary function. This version of the call is used for Views that need to
     * specify a Scroll Range but scroll back to it's edge correctly.
     *
     * @param view - PullToRefreshView that is calling this.
     * @param deltaX - Change in X in pixels, passed through from from overScrollBy call
     * @param scrollX - Current X scroll value in pixels before applying deltaY, passed through from from overScrollBy call
     ...
     * @param scrollY - Current Y scroll value in pixels before applying deltaY, passed through from from overScrollBy call
     * @param scrollRange - Scroll Range of the View, specifically needed for ScrollView
     * @param fuzzyThreshold - Threshold for which the values how fuzzy we should treat the other values. Needed for WebView as it doesn't always
     *            scroll back to it's edge. 0 = no fuzziness.
    ...
    public static void overScrollBy(final PullToRefreshBase<?> view, final int deltaX, final int scrollX,
        ...
        if (view.isPullToRefreshOverScrollEnabled() && !view.isRefreshing()) {
            ...
            if (mode.permitsPullToRefresh() && !isTouchEvent && deltaValue != 0) {
                ...
                pmdStyle(view, deltaX, scrollX, deltaY, scrollY, scrollRange, fuzzyThreshold, scaleFactor, deltaValue, scrollValue,
                        currentScrollValue, mode);
    ...
    private static void pmdStyle(final PullToRefreshBase<?> view, final int deltaX, final int scrollX,
            final int deltaY, final int scrollY, final int scrollRange, final int fuzzyThreshold,
            final float scaleFactor, final int deltaValue, final int scrollValue, final int currentScrollValue,
            final Mode mode) {

        final int newScrollValue = deltaValue + scrollValue;

        if (PullToRefreshBase.DEBUG) {
            Log.d(TAG, "OverScroll. DeltaX: " + deltaX + ", ScrollX: " + scrollX + ", DeltaY: " + deltaY
