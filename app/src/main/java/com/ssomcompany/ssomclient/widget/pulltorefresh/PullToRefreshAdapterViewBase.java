...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T>implements
    ...
    public final void onScroll(final AbsListView view, final int firstVisibleItem, final int visibleItemCount,
        ...
        if (DEBUG) {
            Log.d(TAG, "First Visible: " + firstVisibleItem + ". Visible Count: " + visibleItemCount
    ...
    private boolean isFirstItemVisible() {
        ...
        if (null == adapter || adapter.isEmpty()) {
            ...
            /**
             * This check should really just be: mRefreshableView.getFirstVisiblePosition() == 0, but PtRListView internally use a HeaderView which
             * messes the positions up. For now we'll just add one to account for it and rely on the inner condition which checks getTop().
             */
            if (isPositionLessThanOne(mRefreshableView.getFirstVisiblePosition())) {
    ...
    private boolean isPositionLessThanOne(int position) {
        return position <= 1;
    }
    ...
    private boolean isLastItemVisible() {
        ...
        if (null == adapter || adapter.isEmpty()) {
            ...
            if (DEBUG) {
                Log.d(TAG, "isLastItemVisible. Last Item Position: " + lastItemPosition + " Last Visible Pos: "
                        + lastVisiblePosition);
            }

            /**
             * This check should really just be: lastVisiblePosition == lastItemPosition, but PtRListView internally uses a FooterView which messes
             * the positions up. For me we'll just subtract one to account for it and rely on the inner condition which checks getBottom().
             */
